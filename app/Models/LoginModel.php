...
public function verifyEmail($email)
   {
   	...
   	$builder->select("uni_id,status,username,password");
...
{
  $builder=$this->db->table('users');
  $builder->select("uni_id,username,updated_at");
  $builder->where('uni_id',$token);
  $result=$builder->get();
  if(count($result->getResultArray())==1){return $result->getRowArray();}else{return false;}
}
